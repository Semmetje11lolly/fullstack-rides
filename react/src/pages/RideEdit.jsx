...
function RideEdit() {
    const {setRides, areas} = useContext(AppContext);
    const {id} = useParams();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const areaObj = areas.find(a => a.id === updatedRide.area);

            setRides(prev =>
                prev.map(r =>
                    r.id === updatedRide.id ? {...updatedRide, area: areaObj} : r
                )
            );
    ...
    return (
        <section className={"flex flex-col text-center py-10"}>
            <h1 className={"text-5xl font-bold"}>Edit Attraction</h1>
